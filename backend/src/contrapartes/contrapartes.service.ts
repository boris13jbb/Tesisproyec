...
@Injectable()
export class ContrapartesService {
  constructor(private readonly prisma: PrismaService) {}

  findAll(incluirInactivos: boolean) {
    return this.prisma.contraparte.findMany({
  ...
  async findOne(id: string) {
    const row = await this.prisma.contraparte.findUnique({ where: { id } });
    if (!row) throw new NotFoundException('Contraparte no encontrada');
    return row;
  }

  async create(dto: CreateContraparteDto) {
    const data = normalizePartyInput(dto);
    try {
      return await this.prisma.contraparte.create({ data });
    } catch (e: unknown) {
      if (isPrismaCode(e, 'P2002')) {
        throw new ConflictException(
          'Ya existe una contraparte con esa cédula o RUC',
        );
      }
      throw e;
    }
  ...
  async update(id: string, dto: UpdateContraparteDto) {
    ...
    try {
      return await this.prisma.contraparte.update({
        where: { id },
        data: {
          ...(merged ?? {}),
