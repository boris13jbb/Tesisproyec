...
let DocumentosService = class DocumentosService {
    ...
    async notifyRevisionSubmitted(input) {
    ...
    async enviarRevision(id, viewer, ctx) {
        ...
        const updated = await this.aplicarCambioEstadoSoloEstado(doc, 'EN_REVISION', viewer.id, ctx, { action: 'DOC_SUBMITTED_FOR_REVIEW' });
    ...
    async resolverRevision(id, dto, viewer, ctx) {
        ...
        const updated = await this.aplicarCambioEstadoSoloEstado(doc, dto.decision, viewer.id, ctx, {
