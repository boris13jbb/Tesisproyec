...
export function DependenciasPage() {
  ...
  const [editTarget, setEditTarget] = useState<DependenciaRow | null>(null);

  const load = useCallback(async () => {
    setError(null);
    setLoading(true);
    try {
      const { data } = await apiClient.get<DependenciaRow[]>('/dependencias', {
  ...
  const openEdit = (row: DependenciaRow) => {
    setEditTarget(row);
    editForm.reset({
      nombre: row.nombre,
  ...
  const onCreate = createForm.handleSubmit(async (data) => {
    setError(null);
    try {
      await apiClient.post('/dependencias', {
        codigo: data.codigo,
        nombre: data.nombre,
      ...
      if (isAxiosError(err) && err.response?.data) {
        ...
        setError(
          Array.isArray(m) ? m.join(' ') : (m ?? 'No se pudo crear.'),
        );
      } else {
  ...
  const onEdit = editForm.handleSubmit(async (data) => {
    ...
    setError(null);
    try {
      await apiClient.patch(`/dependencias/${editTarget.id}`, {
        nombre: data.nombre,
  ...
  return (
    <>
      <Box sx={{ width: '100%', pb: { xs: 4, md: 5 } }}>
        ...
        <ListPanel
          badge={<BusinessOutlinedIcon fontSize="small" />}
