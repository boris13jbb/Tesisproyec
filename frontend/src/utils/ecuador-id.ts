...
export function validateRucEcuatoriano(raw: string): boolean {
  ...
  if (ruc.length !== 13 || !/^\d{13}$/.test(ruc)) return false;
...
export function cedulaErrorMessage(raw: string): string | null {
  if (!raw.trim()) return 'Cédula requerida';
  return validateCedulaEcuatoriana(raw) ? null : 'Cédula ecuatoriana inválida';
}

export function rucErrorMessage(raw: string): string | null {
  if (!raw.trim()) return 'RUC requerido';
  return validateRucEcuatoriano(raw) ? null : 'RUC ecuatoriano inválido';
}
