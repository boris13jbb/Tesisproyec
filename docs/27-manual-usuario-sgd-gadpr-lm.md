...
3. Pulse **Consultar** para aplicar filtros y cargar la tabla (antes de consultar, los cambios en los campos no actualizan el listado). El listado muestra **10 registros por página** por defecto. Arriba aparece el bloque **Estadísticas del período filtrado** con: tarjetas KPI (registros, OK, Fallo, docs creados/desactivados, archivos eliminados), gráfico donut **OK vs Fallo**, barras de **acciones por usuario** (top 10) y barras de **acciones sensibles** con semáforo (alta/media/baja, también en leyenda y tooltip). En cada fila, **Ver detalle** abre un diálogo con fecha, usuario, acción, resultado, recurso, origen (IP normalizada) y **Detalle adicional** en etiquetas legibles (decisión, motivo, transición de estado, etc.); **no** se muestra JSON crudo. El listado usa el **icono de auditoría** del menú (no una letra «A») y chips de resultado **Correcto** / **No completado** según el tema. Opcional: icono **Actualizar** en la cabecera para repetir la consulta con los mismos filtros y página actual (también refresca las estadísticas y gráficos).
