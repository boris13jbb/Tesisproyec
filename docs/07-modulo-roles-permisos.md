...
- **Permisos en BD**: `permissions` + `role_permissions` poblados de forma **idempotente** al ejecutar `npx prisma db seed` (desde `backend/`). **`ADMIN`** recibe todos los códigos; **`USUARIO`** recibe consulta, creación, edición, subida y descarga de archivos, y envío a revisión (sin eliminar archivos ni resolver revisión); los demás roles reciben subconjuntos alineados con el seed (`REVISOR` incluye `DOC_REVISION_RESOLVE`, etc.).
...
#### Catálogos
- `CATALOGS_READ`
- `DEPENDENCIAS_WRITE`, `CARGOS_WRITE`, `TIPOS_DOCUMENTALES_WRITE`, `CONTRAPARTES_WRITE`, `BENEFICIARIOS_WRITE`
