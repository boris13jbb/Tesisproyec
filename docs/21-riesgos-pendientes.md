...
| R-006 | Enumeración de cuentas en recuperación de credenciales (revelar si el correo existe) | Alto | Media | Alta | Respuesta constante `202 { ok: true }`; no revelar existencia; registrar evento en auditoría transversal cuando exista | Mitigado (parcial) | Implementado a nivel API; falta bitácora central (`audit_logs`) |
...
| R-011 | Controles incompletos de auditoría transversal (eventos de seguridad/administración) | Alto | Media | Alta | Implementar `audit_logs` central; correlación (`correlationId`), retención, exportación, integridad básica | Abierto | Trazabilidad por dominio existe (`documento_eventos`, `documento_archivo_eventos`) pero no bitácora central |
