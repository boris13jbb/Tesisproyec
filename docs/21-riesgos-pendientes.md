...
Actualizado **2026-05-06** (coherencia con anti‑IDOR en documentos, auditoría UI, lockout y reportes extendidos). *ETAPA 0* cerrada como hito (`docs/29-etapa-0-cierre-y-evidencias.md`). Detalle técnico: `docs/22-changelog-tecnico.md`.
...
| R-007 | Abuso de endpoint de recuperación (fuerza bruta / spam) | Medio | Media | Media | Rate limit (por IP y por email), backoff; CAPTCHA institucional si aplica; monitoreo de picos | Mitigado (parcial) | Implementado `@nestjs/throttler` global + rutas `/auth`; refinar por email como clave adicional si la política lo exige |
...
| R-010 | IDOR (acceso a recursos por UUID sin validar permisos/propiedad) | Alto | Media | Alta | Validar autorización por recurso en cada endpoint; no confiar en IDs del cliente; pruebas negativas | Mitigado (parcial) | **Documentos/adjuntos/reportes:** filtros por `dependencia` + `confidencialidad` para no‑ADMIN; JWT con `dependenciaId`. Persisten riesgos de política fina / permiso granular no modelado (`07`). |
| R-011 | Controles incompletos de auditoría transversal (eventos de seguridad/administración) | Alto | Media | Alta | Completar cobertura por módulo; correlación; retención; exportación ADMIN; integridad básica | Mitigado (parcial) | `audit_logs` + `AuditService`; **UI `/admin/auditoria`** + exports; persisten **retención** institucional y cobertura total opcional de denegaciones. |
...
| R-016 | Falta de respaldos operativos y pruebas de restauración | Alto | Media | Alta | Backups BD + storage + logs; pruebas de restore; RPO/RTO; registro de ejecuciones | Mitigado (parcial) | Dump `mysqldump` + cron/UI + auditoría `BACKUP_VERIFIED`. Restore **fuera del SGD**. Pendiente: cifrado at-rest, off-site, restore-test automático, SHA-256. Ver `MATRIZ_SEGURIDAD_BACKUPS_RECUPERACION.md`. |
...
| R-018 | Falta de segregación de funciones (ADMIN único con todo) | Medio | Media | Media | Separar roles: Admin seguridad, Admin catálogos, Operador, Auditor; mínimo privilegio | Abierto | Existen **`REVISOR`**, **`AUDITOR`**, **`CONSULTA`** en seed y administración de usuarios; la segregación fuerte sigue pendiente hasta `PermissionsGuard` y flujos por rol (`07`). |
| R-019 | Gestión de usuarios incompleta (sin desactivación, sin rotación de roles, sin historial central) | Medio | Media | Media | Endpoint y UI para activar/inactivar; auditoría de cambios; rotación de roles con evidencia | Mitigado (parcial) | **Activar/desactivar** y **asignación de roles** en API + `/admin/usuarios`; auditoría central de todo el historial administrativo pendiente según alcance institucional. |
